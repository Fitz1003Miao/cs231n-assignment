...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (H, x, Wx, prev_h, Wh, b, a, i, f, o, g, prev_c, next_c, next_h)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    H, x, Wx, prev_h, Wh, b, a, i, f, o, g, prev_c, next_c, next_h = cache
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for t in range(T):
      if t == 0:
        h[:, t, :], c, forward_step_cache[t] = lstm_step_forward(x[:, t, :], h0, c, Wx, Wh, b)
      else:
        h[:, t, :], c, forward_step_cache[t] = lstm_step_forward(x[:, t, :], h[:, t - 1, :], c, Wx, Wh, b)
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
      dh_ag = dh[:, t, :] + dprev_h
      dx_step, dprev_h, dprev_c, dWx_step, dWh_step, db_step = lstm_step_backward(dh_ag, dprev_c, forward_step_cache[t])
      dWx += dWx_step
      dWh += dWh_step
      db += db_step
      dx[:, t, :] = dx_step
