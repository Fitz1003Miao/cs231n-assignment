...
for learning_rate in learning_rates:
    for reg in regularization_strengths:
        svm = LinearSVM()
        svm.train(X_train_feats, y_train, learning_rate = learning_rate, reg = reg, num_iters = 1500)
...
for hidden_size in hidden_sizes:
    for learning_rate in learning_rates:
        for reg in regularization_strengths:
            net = TwoLayerNet(input_dim, hidden_dim, num_classes)
            net.train(X_train_feats, y_train, X_val_feats, y_val, learning_rate = learning_rate, reg = reg, num_iters = 2000)
            acc_val = np.mean(net.predict(X_val_feats) == y_val)
            print('for hs: %e, lr: %e and r: %e, valid accuracy is: %f' % (hidden_size, learning_rate, reg, acc_val))
            if acc_val > best_acc:
                best_acc = acc_val
                best_net = net
################################################################################
