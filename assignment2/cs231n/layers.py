...
def affine_forward(x, w, b):
    ...
    # pass
    D = w.shape[0]
    out = np.dot(x.reshape(-1, D), w) + b.reshape(1, -1)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = (x, w, b)
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        # cache = (x_hat, sample_mean, sample_var, eps, gamma, beta, x)
        ...
        var = 1 / N * np.sum(x_mu_square, axis = 0)
        var_sqrt = np.sqrt(var + eps)
        ivar_sqrt = 1 / var_sqrt

        x_norm = x_mu * ivar_sqrt
        out = x_norm * gamma + beta
        cache = (x, mu, x_mu, x_mu_square, var, var_sqrt, ivar_sqrt, x_norm, gamma, beta, eps)
...
def batchnorm_backward(dout, cache):
    ...
    # dx = dx1 + dx2
    N,D = dout.shape
    x, mu, x_mu, x_mu_square, var, var_sqrt, ivar_sqrt, x_norm, gamma, beta, eps = cache
    dout_dbeta = np.sum(dout, axis = 0)
    dout_dgamma = np.sum(dout * x_norm, axis = 0)

    dout_dx_norm = dout * gamma
    dout_dx_mu1 = dout_dx_norm * ivar_sqrt

    dout_divar_sqrt = np.sum(dout_dx_norm * x_mu, axis = 0)
    dout_dvar_sqrt = -1 / np.square(var_sqrt) * dout_divar_sqrt

    dout_dvar = 0.5 * 1 / np.sqrt(var + eps) * dout_dvar_sqrt

    dout_dx_mu_square = 1 / N * np.ones([N, D]) * dout_dvar
    dout_dx_mu2 = 2 * x_mu * dout_dx_mu_square

    dout_dx_mu = dout_dx_mu1 + dout_dx_mu2
    dout_dx1 = dout_dx_mu

    dout_dmu = -1 * np.sum(dout_dx_mu, axis = 0)
    dout_dx2 = 1 / N * np.ones([N, D]) * dout_dmu
    dout_dx = dout_dx1 + dout_dx2
    dx = dout_dx
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    x_mu = x - mu
    x_mu_square = np.square(x_mu)
    var = np.sum(x_mu_square, axis = 1, keepdims = True)
    var_sqrt = np.sqrt(var + eps)
    ivar_sqrt = 1 / var_sqrt
    x_norm = x_mu * ivar_sqrt

    out = x_norm * gamma + beta
    cache = (x, gamma, beta, eps, mu, x_mu, x_mu_square, var, var_sqrt, ivar_sqrt, x_norm)
...
def layernorm_backward(dout, cache):
    ...
    x, gamma, beta, eps, mu, x_mu, x_mu_square, var, var_sqrt, ivar_sqrt, x_norm = cache
    N, D = dout.shape
    ...
    dx_norm = dout * gamma
    dx_mu1 = dx_norm * ivar_sqrt
    divar_sqrt = np.sum(dx_norm * x_mu, axis = 1)
    dvar_sqrt = -1 / np.square(var_sqrt) * divar_sqrt.reshape(-1, 1)

    dvar = 0.5 * 1 / np.sqrt(var + eps) * dvar_sqrt
    dx_mu_square = np.ones([N, D]) * dvar
    dx_mu2 = 2 * x_mu * dx_mu_square
    dx_mu = dx_mu1 + dx_mu2
    
    dx1 = dx_mu
    dmu = -1 * np.sum(dx_mu, axis = 1).reshape(-1, 1)
    dx2 = np.ones([N, D]) * dmu
    dx = dx1 + dx2
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    assert(H - HH + 2 * pad) % stride == 0, "pad error"
    assert(W - WW + 2 * pad) % stride == 0, "pad error"
        ...
        for f in range(F):
            for h in range(H_prime):
                for i in range(W_prime):
                    out[n, f, h, i] = np.sum(x_pad[n, :, h * stride : h * stride + HH, i * stride : i * stride + WW] * w[f, :, :, :]) + b[f]
...
def max_pool_forward_naive(x, pool_param):
    ...
    assert (H - pool_height) % stride == 0, "height or stride error"
    assert (W - pool_width) % stride == 0, "width or stride error"
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(H):
                for j in range(W):
                    temp = np.zeros([pool_height, pool_width])
                    index = np.argmax(x[n, c, i * stride : i * stride + pool_height, j * stride : j * stride + pool_width])
                    temp[index // pool_width, index % pool_width] = 1
                    dx[n, c, i * stride : i * stride + pool_height, j * stride : j * stride + pool_width] += temp * dout[n, c, i, j]
